...
--   ✅ CREATE NETWORK RULE ... MODE = EGRESS — 실제 실행 검증 완료
--      해석 가능한 호스트('example.com:5432')로 생성 성공.
--      ⚠️ 이는 **DNS 검증 동작만** 확인한 것입니다. 실제 PostgreSQL 서버가
--         아니므로 연결 검증은 아닙니다.
--   ✅ EGRESS 규칙이 생성 시점에 DNS 를 검증한다 — 실제 실행으로 확인.
--      해석 불가 호스트로 생성 시도 시 오류 원문:
--        invalid value '[...]' for property 'VALUE_LIST', Reason: One or more
--        values might be an unresolvable host name. Verify all hosts are
--        resolvable, or use a wildcard pattern.
--      → 아래 ⚠️ 경고가 실측으로 확인되었습니다.
--
--   🔴 CREATE EXTERNAL ACCESS INTEGRATION — **트라이얼 계정에서 차단됩니다**
--      오류 원문: "External access is not supported for trial accounts."
--      공식 문서: "By default, Snowflake does not enable external access for
--      trial accounts. Contact your account representative to get external
--      access enabled for a trial account."
--      → **이 지점이 트라이얼 계정에서 이 실습의 실질적 종착점입니다.**
--        Deployment·Runtime·커넥터 생성은 트라이얼에서도 성공합니다(03·06번).
--        해소 방법: ① 신용카드 등록으로 유료 전환 ② 계정 담당자에게 활성화 요청
--
--   ⚠️ 미검증: ALTER OPENFLOW RUNTIME SET EXTERNAL_ACCESS_INTEGRATIONS.
--      EAI 를 만들 수 없어 연결 대상이 없었습니다. 문법은 공식 문서로 대조.
--   ⚠️ 미검증: CREATE SECRET. 실재 소스 자격증명이 없습니다.
-- =============================================================
