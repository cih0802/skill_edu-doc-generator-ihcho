...
--   ✅ STEP 0 사전 스냅샷 조회문 전체 — 실제 실행 검증 완료
--   ✅ CREATE ROLE 3종 및 역할 계층 GRANT — 실제 실행 검증 완료
--   ✅ 계정 레벨 GRANT 전체 — 실제 실행 검증 완료
--      CREATE OPENFLOW DEPLOYMENT / CREATE DATABASE / CREATE INTEGRATION /
--      CREATE COMPUTE POOL 모두 성공 (이전 판의 "GRANT 는 실행 미검증" 해소)
--   ✅ CREATE DATABASE 2종 + SCHEMA + 스키마 레벨 GRANT — 실제 실행 검증 완료
--   ✅ CREATE WAREHOUSE + USAGE, OPERATE GRANT — 실제 실행 검증 완료
--   ✅ CREATE EVENT TABLE — 실제 실행 검증 완료
--   ✅ 전체 DROP 후 기준선 복귀 — 실제 실행 검증 완료
--      (DB 5 / WH 3 / Role 7 / Integration 2 — 실습 전과 일치)
--
--   📌 STEP 1-5 의 ALTER USER ... SET DEFAULT_SECONDARY_ROLES = ('ALL') 은
--      이 계정에서 **실행하지 않았습니다.** DESCRIBE USER 결과가 이미
--      DEFAULT_SECONDARY_ROLES = ["ALL"] (계정 기본값)이어서 변경이 불필요했습니다.
--      → 실습 전 값이 이미 ALL 이면 이 문장을 건너뛰고, 98_ PART C-7 의
--        원복도 수행하지 않는 것이 맞습니다. STEP 0-6 (d) 에서 먼저 확인하십시오.
-- =============================================================
