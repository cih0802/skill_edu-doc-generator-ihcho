...
| 95 | `95_실습전_기준선.md` | MD | 실습 전 기준선 (사전 스냅샷) | 실습 시작 전 계정 상태와 변경 대상 사용자 속성을 기록하는 조회 전용 쿼리와 빈 기록 서식. 정리 후 96_와 대조해 왕복 가능성을 증명하는 기준이 된다. |
| 96 | `96_실습후_대조.md` | MD | 실습 후 대조 (왕복 검증) | 정리 실행 후 95_에 기록한 기준선과 같은 쿼리를 재실행해 잔여물 0과 속성 원복을 확인하는 대조 서식. 이 문서가 왕복 검증의 정본이다. |
...
95. **`95_실습전_기준선.md`** — 실습 전 기준선 (사전 스냅샷) ⚠️ *Snowflake 외부/혼합 작업*
   - 실습 시작 전 계정 상태와 변경 대상 사용자 속성을 기록하는 조회 전용 쿼리와 빈 기록 서식. 정리 후 96_와 대조해 왕복 가능성을 증명하는 기준이 된다.
96. **`96_실습후_대조.md`** — 실습 후 대조 (왕복 검증) ⚠️ *Snowflake 외부/혼합 작업*
   - 정리 실행 후 95_에 기록한 기준선과 같은 쿼리를 재실행해 잔여물 0과 속성 원복을 확인하는 대조 서식. 이 문서가 왕복 검증의 정본이다.
...
| `02_사전준비_및_권한구성.sql` | 95_실습전_기준선.md | 03_openflow_배포_및_런타임_생성.sql |
...
| `95_실습전_기준선.md` | 없음 | 02_사전준비_및_권한구성.sql |
| `96_실습후_대조.md` | 98_리소스정리.sql | 없음 |
| `98_리소스정리.sql` | 09_적재결과_검증.sql | 96_실습후_대조.md |
