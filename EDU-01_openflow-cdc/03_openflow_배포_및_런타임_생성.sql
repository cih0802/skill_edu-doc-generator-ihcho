...
--
-- 검증 상태 (2026-09-16, 계정 LJ20513 / AWS_AP_NORTHEAST_1 / Enterprise 트라이얼
--            에서 실제 실행):
--   ✅ CREATE OPENFLOW DEPLOYMENT ... DEPLOYMENT_TYPE = SNOWFLAKE — 실제 실행 검증 완료
--      status: CREATING → ACTIVE (약 12분)
--   ✅ SYSTEM$WAIT_FOR_STABLE_OPENFLOW_DEPLOYMENTS — 실제 실행 검증 완료
--   ✅ ALTER OPENFLOW DEPLOYMENT SET EVENT_TABLE — 실제 실행 검증 완료
--   ✅ GRANT USAGE ON OPENFLOW DEPLOYMENT — 실제 실행 검증 완료
--   ✅ CREATE OPENFLOW RUNTIME (NODE_TYPE=SMALL, TIER='S1', 1노드) — 실제 실행 검증 완료
--      status: CREATING → ACTIVE (약 3분)
--   ✅ ALTER OPENFLOW RUNTIME SET NODE_TYPE_TIER = 'S2' — 실제 실행 검증 완료 (변경 가능)
--   ✅ NODE_TYPE 은 변경 불가 — 실제 실행으로 확인
--      ALTER ... SET NODE_TYPE = MEDIUM →
--      "invalid property 'NODE_TYPE' for 'OPENFLOW RUNTIME'"
--   ✅ SUSPEND → TERMINATE → DROP 워크플로 — 실제 실행 검증 완료 (98_ 참고)
--   ✅ Deployment 도 TERMINATE → DROP 이 필요함 — 실제 실행으로 확인 (아래 🔴)
--
--   🔴 트라이얼 계정에 대한 이전 판의 경고는 **대상이 틀렸습니다.**
--      `CREATE OPENFLOW DEPLOYMENT` 는 이 트라이얼 계정에서 **성공했습니다.**
--      실제 트라이얼 차단 지점은 **05번 문서의 External Access Integration** 입니다.
--        CREATE EXTERNAL ACCESS INTEGRATION →
--        "External access is not supported for trial accounts."
--      공식 문서: "By default, Snowflake does not enable external access for
--      trial accounts. Contact your account representative..."
--      → 즉 트라이얼에서는 Deployment·Runtime·커넥터까지는 만들 수 있으나
--        **외부 소스에 연결할 수 없어 CDC 적재를 완주할 수 없습니다.**
--
--   ⚠️ 미검증: 실제 CDC 적재(07~09번). 소스 DB 연결이 EAI 차단으로 불가.
-- =============================================================
...
-- 안정 상태에 도달하면 문자열 'OK' 를 반환합니다 (실측). 실패/타임아웃 시 예외.
--   반환값: ______
--
...
-- 검증
DESCRIBE OPENFLOW DEPLOYMENT OPENFLOW_EDU_DEPLOYMENT;
-- ⚠️ DESCRIBE / SHOW 출력에는 **EVENT_TABLE 열이 없습니다** (2026-09-16 실측).
--    따라서 이 출력으로는 설정 여부를 확인할 수 없습니다. status = ACTIVE 만
--    확인하고, 실제 확인은 Runtime 기동 후 Event Table 에 행이 적재되는지로
--    판단하십시오.
--   status: ______
--
