-- ==============================================================================
-- [Step 6] 모바일 앱 연동 External OAuth 및 챗봇 API 보안 설정 (검증 완료)
...
-- 6.1 모바일 백엔드 API 서버 (BFF) 연계용 External OAuth Security Integration 생성
...
CREATE OR REPLACE SECURITY INTEGRATION MOBILE_APP_OAUTH_INTEGRATION
    TYPE = EXTERNAL_OAUTH
    ENABLED = TRUE
    ...
    COMMENT = '사내 모바일 앱 백엔드 및 현장직 사용자용 External OAuth 연동';

-- 6.2 모바일 BFF 백엔드 서비스 계정 생성 (Key-Pair / Service Account)
CREATE OR REPLACE USER KSM_MOBILE_CHATBOT_SVC_USER
    ...
    TYPE = SERVICE
    COMMENT = '모바일 챗봇 백엔드 게이트웨이 API 전용 서비스 계정';

GRANT ROLE KSM_MOBILE_CHATBOT_SERVICE_ROLE TO USER KSM_MOBILE_CHATBOT_SVC_USER;

-- 6.3 현장직 및 사무직 전용 AI 서비스(Cortex / Search) 사용 권한 부여
...
-- 6.4 현장직 전용 지식 베이스 매뉴얼 테이블 생성 (GOLD 스키마)
...
COMMENT = '현장 생산/설비 작업자 전용 지식 베이스 매뉴얼';
...
SELECT 'Step 6 Completed: Mobile App External OAuth & Chatbot Security Configured and Verified' AS STATUS;
