-- ==============================================================================
-- [Step 4] 현장직 인사 DB 동기화 테이블 및 CDC Stream 파이프라인
...
--
-- 📌 위 스트림 액션 표는 Step 5 동기화 로직의 설계 근거입니다. 5번 문서 상단 참고.
--    · UPDATE 는 DELETE + INSERT 쌍을 만든다 → INSERT 행 존재 여부로 하드 삭제를 판별할 수 있다
--    · 순수 DELETE 는 DELETE 1행만 만든다 → 이 경우가 오프보딩 대상이다
--    이전 판의 Step 5 는 METADATA$ACTION='INSERT' 만 필터해 순수 DELETE 를 놓쳤고
...
-- 4.1 사내 인사 DB 마스터 스테이징 테이블 생성 (BRONZE 스키마)
...
COMMENT = '사내 인사 시스템(현장직)에서 주기적으로 동기화되는 임직원 원장 테이블';

-- 4.2 동기화 작업 결과 감사(Audit) 로그 테이블 (SILVER 스키마)
...
COMMENT = '인사 DB -> Snowflake 사용자 및 권한 동기화 이력 감사 로그';

-- 4.3 변경 데이터 캡처 (CDC)를 위한 Stream 생성 (BRONZE 스키마)
...
CREATE OR REPLACE STREAM KSM_ENTERPRISE_DB.BRONZE.HR_EMPLOYEE_STREAM
    ...
    APPEND_ONLY = FALSE
    COMMENT = '인사 마스터 테이블 변경분 감지 스트림';

-- 4.4 기초 테스트 데이터 적재 (현장 작업자 샘플)
...
SELECT 'Step 4 Completed: HR Master Table and Stream Successfully Created' AS STATUS;
