...
step: 08
type: sql
summary: 신규 입사, 퇴사(상태 변경), 하드 삭제, 재실행 차단, 자정 감사 시나리오를 순차 실행해 자동 프로비저닝 결과를 확인한다. 리소스 정리는 98_ 이 정본이다.
requires: 06_사용자권한_자동동기화_프로시저_및_태스크.sql
next: 98_리소스정리.sql
*/
...
--      06_ 문서 [결함 2] 로 정정했고, 수정형은 실제 실행으로 검증했다(1명 조치).
--   ✅ 정리 절차 전체 — 실제 실행 검증 완료 (당시 이 문서의 7.4 절, 현재 98_ 로 이관)
...
--    따라서 8.0 에서 태스크를 SUSPEND 한 뒤 시나리오를 진행하십시오.
--
-- 🔴 정리는 선택이 아니라 **필수**입니다. 이전 판이 7.4 절에 "[선택 실행]" 으로
--    표기했던 것을 정정하고 98_리소스정리.sql 로 이관했습니다.
--    건너뛰면 RESUME 된 태스크 2개, SCIM 베어러 토큰, 계정 레벨 권한이 남습니다.
-- ==============================================================================
...
-- ==============================================================================
-- 8.0 시나리오 검증 전 태스크 일시 중지 (결정적 관찰을 위해 필수)
-- ==============================================================================
...
-- ==============================================================================
-- 8.1 [시나리오 1] 신규 현장 직원 입사 (EMP_F004 추가)
-- ==============================================================================
...
-- ==============================================================================
-- 8.2 [시나리오 2] 현장 직원 퇴사 발생 (EMP_F001 퇴사 처리)
-- ==============================================================================
...
-- ==============================================================================
-- 8.2-1 [시나리오 2-2] 인사 원장에서 행이 삭제된 경우 (하드 삭제 오프보딩)
-- ==============================================================================
...
-- ==============================================================================
-- 8.2-2 [확인] 스트림 소비로 재실행이 차단되는지 (5번 [결함 1] 정정 확인)
-- ==============================================================================
...
-- ==============================================================================
-- 8.3 [시나리오 3] 매일 자정 퇴사자 잔존 점검 및 보안 감사 프로시저 수동 실행
-- ==============================================================================
...
-- 🧹 리소스 정리
-- ==============================================================================
-- 🔴 이 문서에는 정리 구문을 두지 않습니다. **정본은 `98_리소스정리.sql` 입니다.**
--    이전 판은 정리 절차를 이 문서의 7.4 절에 `[선택 실행]` 으로 두었습니다.
--    두 가지 문제가 있었습니다.
--      ① 실질적으로 **필수**인데 선택으로 표기되어 건너뛰기 쉬웠다
--      ② 정리 절차가 시나리오 문서에 묻혀 있어 실행 가능한 정본이 없었다
--    → `98_리소스정리.sql` 로 이관했습니다.
--
-- 이 문서가 만든 것 (원장 데이터 변경분):
--   · HR_EMPLOYEE_MASTER 의 EMP_F004 / EMP_F005 행 추가
--   · EMP_F001 상태 RESIGNED 변경, EMP_F002 부서 변경, EMP_F003 행 삭제
--   · 위 변경으로 동기화 프로시저가 만든 사용자 EMP_F001~EMP_F005
--   → 모두 `98_리소스정리.sql` 의 사용자 삭제 및 DB 삭제로 제거됩니다.
--
-- ⚠️ 실습을 여기서 중단하더라도 **반드시 `98_리소스정리.sql` 을 실행하십시오.**
--    RESUME 된 서버리스 태스크 2개, SCIM 베어러 토큰, 계정 레벨 권한 위임이
--    남습니다. 태스크는 계정을 자동으로 DISABLED 시키는 동작을 계속합니다.

SELECT 'Step 08 Completed: 동기화 시나리오 검증 완료. 다음은 98_리소스정리.sql 입니다.' AS STATUS;
